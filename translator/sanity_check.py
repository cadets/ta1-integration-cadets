...
def examine_file(path):
    ...
    with open(path, 'r') as cadets_in:
        ...
        while 1:
            ...
            if not validate_record(cadets_record):
...
def validate_record(record):
    ...
    '''
    if not record.get("datum"): # Everything should have a datum
        return False

    details = record["datum"]

    if details.get("baseObject"):
        return validate_file(details)

    if not details.get("type"): # Everything but files should have a "type"
    ...
    if record_type.startswith("EVENT"):
        return validate_event(details)
    elif record_type.startswith("EDGE"):
        return validate_edge(details)
    elif record_type == "SUBJECT_PROCESS":
        return validate_process(details)
    elif record_type == "PRINCIPAL_LOCAL":
        return validate_principal(details)
